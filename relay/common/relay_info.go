...
func genBaseRelayInfo(c *gin.Context, request dto.Request) *RelayInfo {
	...
	if common.OpenWebUIUserIntegrationEnabled && common.OpenWebUIUserIntegrationFunc != nil && c.Request.Header.Get("X-OpenWebUI-User-Email") != "" {
		// 查询用户是否已经注册
		...
		// 如果已经注册，改写用户信息
		if err == nil && userId != 0 { // FillUserByEmail静默忽略错误，我们必须再检查一下用户是否存在
